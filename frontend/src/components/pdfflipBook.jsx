      ...
      const uploadPromise = new Promise((resolve, reject) => {
        xhr.addEventListener('load', () => {
          if (xhr.status >= 200 && xhr.status < 300) {
            ...
            reject(new Error(JSON.parse(xhr.responseText).message || 'This Email is already registered.Please use another email.'));
          }
        });
...
<label className={`
  ...
  ${isDarkMode 
    ? 'bg-[#8e048e] hover:bg-[#a706a7] text-white'  // Your exact color
    : 'bg-[#7b0872] hover:bg-[#8e048e] text-white'  // Your exact color
  }
`}>
